      ...
      <div className="w-full rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-900 via-slate-800 to-sky-900 text-white shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 px-4 py-4">
          {items.map((item) => (
            <div
              key={item.title}
              className="relative overflow-hidden flex flex-col rounded-xl bg-white/5 px-4 py-3 min-h-[92px]"
            >
              ...
              {item.payments?.length ? (
                <div className="mt-6 flex flex-wrap items-center gap-6 text-white/80">
                  {item.payments.includes("klarna") ? <span className="flex items-center justify-center rounded-md border border-white/60 bg-[#FFB3C7] px-3 py-2 border border-white/60">
                    <SiKlarna className="text-slate-900 text-2xl" />
                  </span> : null}
                  {item.payments.includes("visa") ? <span className="flex items-center justify-center rounded-md border border-white/60 bg-[#1A1F71] px-3 py-2 border border-white/60">
                    <SiVisa className="text-slate-100 text-2xl" />
                  </span> : null}
                  {item.payments.includes("mastercard") ? <span className="flex items-center justify-center rounded-md border border-white/60 bg-white/95 px-1 py-2 shadow-sm">
                    <Image
                  ...
                  {item.payments.includes("paypal") ? <span className="flex items-center justify-center rounded-md border border-white/60 bg-white/95 px-3 py-2 shadow-sm">
              ...
              {item.showReturnsIcon ? (
                <RotateCcw 
                strokeWidth={2}
                className="absolute -top-0 -right-1 h-24 w-24 text-slate-400/30 md:text-slate-500 pointer-events-none" />
              ) : null}
            </div>
          ))}
        </div>
